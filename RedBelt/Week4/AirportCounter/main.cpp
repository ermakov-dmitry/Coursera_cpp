...
public:
    ...
    AirportCounter(TIterator begin, TIterator end) {
        for (auto it = begin; it != end; it++) {
            data[static_cast<size_t>(*it)].first = *it;
    ...
    void EraseAll(TAirport airport) {
        data[static_cast<size_t>(airport)].second = 0u;
    }
    ...
    Items GetItems() const {
        ...
        for (size_t idx = 0; idx < data.size(); idx++) {
            result[idx].first = static_cast<TAirport>(idx);
        }
        return result;
    }
...
/*    auto x = airport_counter.GetItems();  // test
    return;*/
