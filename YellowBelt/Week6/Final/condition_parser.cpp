...
shared_ptr<Node> ParseComparison(It& current, It end) {
    ...
    Token& op = *current;
    if (op.type != TokenType::COMPARE_OP) {  // тут все верно
        throw logic_error("Expected comparison operation");
    }
    ++current;
    ...
    const string& value = current->value;  //value == "2017-01-01"
    ++current; // переходим на {"AND", TokenType::LOGICAL_OP}
