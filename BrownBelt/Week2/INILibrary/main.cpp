...
void TestEmptyDocumentWithNewlines() {
    std::istringstream input(
            "\n\n\n\n"
    );
...
void TestEmptySection() {
    ...
    auto doc = Ini::Load(input);
    ASSERT_EQUAL(doc.SectionCount(), 3);
}
...
int main() {
    ...
    RUN_TEST(tr, TestEmptyDocument);
    RUN_TEST(tr, TestEmptyDocumentWithNewlines);
    RUN_TEST(tr, Test1);
    RUN_TEST(tr, TestEmptySection);
    return 0;
}