...
private:
    ...
    class IndexProxy {
    public:
        IndexProxy(Polynomial& poly, const size_t& degree) : poly_(poly), degree_(degree) {}
        IndexProxy& operator=(const T& value) {
            if (value == 0) {
                if (degree_ > poly_.coeffs_.size() - 1) {
                    return *this;
                } else {
                    poly_.coeffs_[degree_] = value;
                    auto it = find_if(poly_.coeffs_.rbegin(), poly_.coeffs_.rend(), [](const T& x) {return x != 0;});
                    poly_.coeffs_.erase(it.base(), poly_.coeffs_.end());
                }
            } else if (degree_ > poly_.coeffs_.size() - 1){
                poly_.coeffs_.resize(degree_ + 1);
            }
            poly_.coeffs_[degree_] = value;
            poly_.coeffs_.shrink_to_fit();
            return *this;
        }
        operator T() const {
