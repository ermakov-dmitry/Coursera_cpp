...
public:
    ...
    struct WriteAccess {
        explicit WriteAccess(mutex& mtx, const K& key, MapType& kv) : guard(mtx), ref_to_value(kv[key]) {}
        lock_guard<mutex> guard;
        V& ref_to_value;
    };

    struct ReadAccess {
        explicit ReadAccess(mutex& mtx, const K& key, const MapType& kv) : guard(mtx), ref_to_value(kv.at(key)) {}
        lock_guard<mutex> guard;
        const V& ref_to_value;
    };

    explicit ConcurrentMap(size_t bucket_count) : data(vector<MapType>(bucket_count)), mtx(vector<mutex>(bucket_count)) {}

    WriteAccess operator[](const K& key) {
        return WriteAccess(mtx[hasher(key) % mtx.size()], key, data[hasher(key) % data.size()]);
    }
    ReadAccess At(const K& key) const {
        return ReadAccess(mtx[hasher(key) % mtx.size()], key, data[hasher(key) % data.size()]);
    }

    bool Has(const K& key) const {
        lock_guard g(mtx[hasher(key) % mtx.size()]);
        return data[hasher(key) % mtx.size()].count(key);
    }
